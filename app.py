...
# Correct spacing (4 spaces)

@st.cache_resource
def initialize_scraper():
    scraper = Nitter(log_level=1)
    if not scraper.working_instances:
        scraper = Nitter(instances=["https://nitter.poast.org", "https://nitter.privacydev.net"], log_level=1)
    return scraper
...
#     model, vectorizer = load_model_and_vectorizer()  # <-- FIXED HERE
...
def main():
    ...
    if option == "Input text":
        ...
        if st.button("Fetch Tweets"):
            ...
            # ⬇️⬇️ ADD THIS CHECK inside Fetch Button Press
            if not scraper.working_instances:
                scraper = Nitter(instances=["https://nitter.poast.org", "https://nitter.privacydev.net"], log_level=1)
